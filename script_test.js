...
describe("Slice 1: Clicking & Incrementing Coffee", function() {
  ...
  describe("The updateCoffeeView function", function() {
    it.only("calls document.getElementById() or document.querySelector()", function() {
    ...
    it.only("updates the coffee counter to display the current coffee count", function() {
  ...
  describe("The clickCoffee function", function() {
    it.only("increments the coffee count by one", function() {
    ...
    it.only("updates the coffee counter element with the incremented value", function() {
      const coffeeCounter = document.getElementById("coffee_counter");
...
describe("Slice 2: Unlocking & Rendering Producers", function() {
  describe("The unlockProducers function", function() {
    ...
    it.only("changes `unlocked` to `true` when the player's coffee count is equal to or larger than half the initial price of the producer", function() {
      data.coffee = 100;
    ...
    it.only("does not set `unlocked` to `false` once a producer has been unlocked, even if the coffee count drops again", function() {
      data.coffee = 100;
      code.unlockProducers(data.producers, data.coffee);
      data.coffee = 0;
      code.unlockProducers(data.producers, data.coffee);
  ...
  describe("The getUnlockedProducers function", function() {
    ...
    it.only("returns an array of producer objects", function() {
    ...
    it.only("filters out producer objects which are not unlocked", function() {
    ...
    it.only("does not mutate the data", function() {
  ...
  describe("The makeDisplayNameFromId function", function() {
    it.only("returns a string", function() {
      const result = code.makeDisplayNameFromId("input_string");
      expect(result).to.be.a("string");
    });

    it.only("transforms its input string from snake_case to Title Case", function() {
      const testStrings = [
        "input_string",
        "mr._coffee",
        "Mr._Coffee",
  ...
  describe("The makeProducerDiv function", function() {
    ...
    it.only("returns a DOM element", function() {
      const result = code.makeProducerDiv(producer);
      expect(result).to.be.an("HTMLDivElement");
    });

    it.only("correctly fills in template string", function() {
  ...
  describe("The deleteAllChildNodes function", function() {
    ...
    it.only("calls the `.removeChild()` method on the dom node passed in at least once", function() {
    ...
    it.only("gets rid of all of the children of the DOM node passed in", function() {
      code.deleteAllChildNodes(doc.body);
  ...
  describe("The renderProducers function", function() {
    ...
    it.only("appends some producer div elements to the producer container", function() {
      code.renderProducers(data);
    ...
    it.only("unlocks any locked producers which need to be unlocked", function() {
      code.renderProducers(data);
    ...
    it.only("only appends unlocked producers", function() {
      code.renderProducers(data);
      const producerContainer = document.getElementById("producer_container");
    ...
    it.only("deletes the producer container's children before appending new producers", function() {
      const producerContainer = document.getElementById("producer_container");
    ...
    it.only("is not in some way hardcoded to pass the tests", function() {
...
describe("Slice 3: Buying Producers & Tick", function() {
  describe("The getProducerById function", function() {
    ...
    it.only("returns an object", function() {
      const result = code.getProducerById(data, "producer_A");
      expect(result).to.be.an("object");
    });
    it.only("returns the correct producer object", function() {
    ...
    it.only("is not hardcoded to pass the tests", function() {
  ...
  describe("The canAffordProducer function", function() {
    ...
    it.only("returns a boolean", function() {
      const result = code.canAffordProducer(data, "producer_A");
      expect(result).to.be.a("boolean");
    });
    it.only("returns true if the player can afford the producer", function() {
      const result = code.canAffordProducer(data, "producer_A");
      expect(result).to.be.equal(true);
    });
    it.only("returns false if the player cannot afford the producer", function() {
  ...
  describe("The updateCPSView function", function() {
    ...
    it.only("updates the total cps indicator to display the current total cps", function() {
  ...
  describe("The updatePrice function", function() {
    it.only("returns an integer, not a float", function() {
      const result = code.updatePrice(501);
      expect(result).to.be.a("number");
      expect(result % 1).to.be.equal(0);
    });
    it.only("returns 125% of the input price, rounded down", function() {
      const result = code.updatePrice(501);
  ...
  describe("The attemptToBuyProducer function", function() {
    ...
    it.only("increments the quantity of the producer in question only if the player can afford it", function() {
    ...
    it.only("decrements the player's coffee by the *current* price of the producer, but only if the player can afford it", function() {
      code.attemptToBuyProducer(data, "producer_B");
    ...
    it.only("updates the price of the producer to 125% of the previous price, rounded down, but only if the player can afford the producer", function() {
      code.attemptToBuyProducer(data, "producer_A");
    ...
    // hint: use a function already written
    it.only("updates the total CPS, but only if the player can afford the producer", function() {
      code.attemptToBuyProducer(data, "producer_A");
    ...
    it.only("does not modify data in any way if the player tries to buy something they can't afford", function() {
      const snapshot = JSON.stringify(data);
      code.attemptToBuyProducer(data, "producer_B");
      expect(JSON.stringify(data)).to.equal(snapshot);
    });
  ...
  describe("The buyButtonClick function", function() {
    ...
    it.only("mutates the data only if the player can afford the producer", function() {
    ...
    it.only('shows an alert box with the message "Not enough coffee!" only if the player cannot afford the producer', function() {
    ...
    it.only("does not modify data or show an alert box if the event passed in doesn't represent a click on a button element", function() {
      const spyOnAlert = sinon.spy(window, "alert");
      const snapshot = JSON.stringify(data);
    ...
    it.only("renders the updated producers when a purchase succeeds", function() {
      const event = { target: { tagName: "BUTTON", id: "buy_producer_A" } };
      code.buyButtonClick(event, data);
      const producerContainer = document.getElementById("producer_container");
    ...
    // Hint: call a function you've already written!
    it.only("updates the coffee count on the DOM, reflecting that coffee has been spent, when a purchase succeeds", function() {
      const event = { target: { tagName: "BUTTON", id: "buy_producer_A" } };
      code.buyButtonClick(event, data);
      const coffeeCounter = document.getElementById("coffee_counter");
    ...
    // Hint: call a function you've already written!
    it.only("updates the total CPS on the DOM, reflecting that the new producer's CPS has been added", function() {
      const event = { target: { tagName: "BUTTON", id: "buy_producer_A" } };
      code.buyButtonClick(event, data);
      const cpsIndicator = document.getElementById("cps");
  ...
  describe("The tick function", function() {
    ...
    it.only("increases coffee count by the total CPS", function() {
      code.tick(data);
      expect(data.coffee).to.be.equal(100);
    });

    // Hint: use what you've written already! The tick function can be just a few lines
    it.only("updates the dom to reflect this new coffee count", function() {
      code.tick(data);
      const coffeeCounter = document.getElementById("coffee_counter");
      expect(coffeeCounter.innerText).to.equal(100);
    });

    it.only("updates the dom to reflect any newly unlocked producers", function() {
      code.tick(data);
      const producerContainer = document.getElementById("producer_container");
      expect(producerContainer.childNodes.length).to.be.equal(2);
    });
